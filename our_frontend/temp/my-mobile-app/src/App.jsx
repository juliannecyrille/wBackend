...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`https://wbackend-l7y3.onrender.com/server/`)
      .then(res => {
        if (!res.ok) {
  ...
  const uploadFile = async (file, docName, attachmentType) => {
      ...
      try {
          // Changed to relative path
          const response = await fetch(`https://wbackend-l7y3.onrender.com/api/incoming.uploadDocumentAttachment`, {
              method: 'POST',
              body: formData,
          });
  ...
  const performAutofillSearch = useCallback(async () => {
    ...
    try {
      ...
      // Changed to relative path
      const response = await fetch(`https://wbackend-l7y3.onrender.com/api/studentCredentials/search?${queryParams}`); // Corrected path
  ...
  const handleSubmitRequest = async () => {
    ...
    if (!requestFormRes.ok) {
      ...
      for (const docName in selectedDocuments) {
        ...
        // Changed to relative path
        const docReqRes = await fetch(`https://wbackend-l7y3.onrender.com/api/incoming.documentRequested`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    // Changed to relative path
    const receiptRes = await fetch(`https://wbackend-l7y3.onrender.com/api/incoming.receiptDetailsSubmission`, {
      method: 'POST',
